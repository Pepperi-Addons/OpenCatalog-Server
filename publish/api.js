...
function callElasticSearchLambda(endpoint, method, body, contentType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const AWS = awsSdk__default['default'];
...
var dist_1 = dist$1.callElasticSearchLambda;

//#region public method - open catalog api
async function single_item(client, request) {
    // https://papi.pepperi.com/v1.0/open_catalog/items/{itemUUID}
    const uuid = request.query.UUID;
    const subType = C__git_Wsim_Wsim_BackEnd_OpenCatalog_serverSide_node_modules_jwtDecode_lib(client.OAuthAccessToken)['pepperi.addonkey'];
    const body = {
        "query": {
            "bool": {
                "must": [
    ...
    const index = C__git_Wsim_Wsim_BackEnd_OpenCatalog_serverSide_node_modules_jwtDecode_lib(client.OAuthAccessToken)['pepperi.distributoruuid'];
    const endpoint = index + '/_search';
    const method = 'POST';
    const lambdaResponse = await dist_1('oc-' + endpoint, method, JSON.stringify(body));
    let response;
...
async function items(client, request) {
...
async function filters(client, request) {
    ...
    distinct_fields.forEach(field => {
        ...
        if (newWhere == where || newWhere.length == where.length) {
            ...
            listPromises.push(dist_1('oc-' + endpoint, method, JSON.stringify(newBody)));
        }
    });
    ...
    await Promise.all(listPromises).then(function (res) {
        var i = 0;
        while (i < res.length) {
            let filter = {};
...
async function configurations(client, request) {
...
function populateItem(item) {
    ...
    if (item._source.hasOwnProperty("NonIndexedString")) {
        (_a = item._source["NonIndexedString"]) === null || _a === void 0 ? void 0 : _a.forEach(field => {
    ...
    if (item._source.hasOwnProperty("NonIndexedNumber")) {
        (_b = item._source["NonIndexedNumber"]) === null || _b === void 0 ? void 0 : _b.forEach(field => {
            item._source[field.APIName + ".Value"] = field.Value;
        });
        delete item._source.NonIndexedNumber;
    }
    if (item._source.hasOwnProperty("NonIndexedBool")) {
        (_c = item._source["NonIndexedBool"]) === null || _c === void 0 ? void 0 : _c.forEach(field => {
            item._source[field.APIName + ".Value"] = field.Value;
        });
        delete item._source.NonIndexedBool;
    }
    if (item._source.hasOwnProperty("NonIndexedDate")) {
        (_d = item._source["NonIndexedDate"]) === null || _d === void 0 ? void 0 : _d.forEach(field => {
            item._source[field.APIName + ".Value"] = field.Value;
        });
...
exports.items = items;
exports.single_item = single_item;
